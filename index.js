...
for (const file of commandFiles) {
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `[AVISO] O arquivo de comando ${file} está mal formatado ou faltando propriedades essenciais (data, execute, data.name). Ele será ignorado.`
      );
    }
  } catch (error) {
...
client.once("ready", async () => {
  ...
  const commandsToRegister = Array.from(client.commands.values()).map((cmd) =>
    cmd.data.toJSON()
  );
  ...
  try {
    ...
    if (GUILD_ID && GUILD_ID.length > 5) {
      // Um ID de guild tem pelo menos 18 dígitos, 5 é um valor seguro
      const guild = await client.guilds.fetch(GUILD_ID).catch((err) => {
        // <--- USANDO GUILD_ID DA VAR. DE AMBIENTE
      ...
      if (guild) {
        ...
        console.warn(
          `Guild com ID "${GUILD_ID}" não encontrada. Verifique se o bot está no servidor e o ID está correto.`
        );
      }
    } else {
      console.warn(
        "Variável de ambiente GUILD_ID não configurada corretamente. Comandos não serão sincronizados em uma guild específica."
      );
      console.warn(
        "Para sincronizar globalmente (e demorar mais), comente a parte de sincronização de guild."
      );
    }
  } catch (error) {
...
client.login(DISCORD_TOKEN).catch((error) => {
  ...
  // Manter o processo vivo para testes
  setInterval(() => {
    console.log("Bot em modo de teste - aguardando configuração correta...");
